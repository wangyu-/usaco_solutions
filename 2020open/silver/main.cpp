...
int main() 
{
	...
	sort(nd.begin(),nd.end(),cmpx);
	sort(d.begin(),d.end(),cmpx);
	int pre=0;
	int min_y=MAXC;
	int component=d.size();
	for(int i=0;i<d.size();i++)
	{
		if(i!=0)
		{
			...
			{	
				if(test) printf("!!!\n");
				if(find(d[i-1].num!=find(d[i].num)))
				{
					link(d[i-1].num,d[i].num);
					component--;
				}	
			}
		}
		if(test)printf("<num=%d>",d[i].num);
		int j;
		for(j=pre;j<nd.size();j++)
		{
			if(nd[j].x>d[i].x) break;
			if(nd[j].y<min_y) min_y=nd[j].y;
			if(test)printf("<add %d>",nd[j].num);
		}	
		pre=j;
		if(test)printf("<miny:%d>",min_y);
	}
	printf("%d\n",component);
/*
