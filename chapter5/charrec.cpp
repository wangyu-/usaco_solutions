...
int cal1_loop(int start,int &score)
{
    int best=9999;
    int idx=-1;
    for(int i=0;i<27;i++)
...
int cal_miss_loop(int start,int &score)
{
    int best=9999;
    int idx=-1;
    for(int i=0;i<27;i++)
    {
	int r=cal_miss(i,start);
	//printf("<%d>",r);
	if(r<best)
	{
	    best=r;
...
int cal_dup_loop(int start,int &score)
{
    int best=9999;
    int idx=-1;
    for(int i=0;i<27;i++)
    {
	int r=cal_dup(i,start);
	if(r<best)
	{
	    best=r;
	    idx=i;
